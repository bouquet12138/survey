 ...
 * 权限申请的地方
 */
public abstract class SurveyPermissionActivity extends MVPBaseActivity {

    protected final int PERMISSIONS_READ_CONTACTS = 0;//读取本地图片
    protected final int PERMISSIONS_LOCATION_CODE = 1;//定位权限
    protected final int PERMISSIONS_CAMERA = 2;//相机权限
    ...
    protected void selectImageAuthor() {
        ...
        if (ContextCompat.checkSelfPermission(SurveyPermissionActivity.this, Manifest.permission.READ_EXTERNAL_STORAGE) != PackageManager.PERMISSION_GRANTED)
            permissionList.add(Manifest.permission.READ_EXTERNAL_STORAGE);//读取权限
        if (ContextCompat.checkSelfPermission(SurveyPermissionActivity.this, Manifest.permission.WRITE_EXTERNAL_STORAGE) != PackageManager.PERMISSION_GRANTED)
       ...
       /* if (ContextCompat.checkSelfPermission(NewSurveyPermissionActivity.this, Manifest.permission.CAMERA) != PackageManager.PERMISSION_GRANTED)
        ...
        if (permissionList.size() != 0) {
            ...
            for (String str : permissionList) {
                if (ActivityCompat.shouldShowRequestPermissionRationale(SurveyPermissionActivity.this, str)) {
                    Toast.makeText(SurveyPermissionActivity.this, "用户拒绝了权限，不能读取本地图片，" +
                            "请到设置界面打开", Toast.LENGTH_SHORT).show();
                    return;//返回出去
                }
            }
            ActivityCompat.requestPermissions(SurveyPermissionActivity.this,
     ...
     * 相机拍照权限
     */
    protected void takePhotoAuthor() {
        List<String> permissionList = new ArrayList<>();//权限组
        if (ContextCompat.checkSelfPermission(SurveyPermissionActivity.this, Manifest.permission.READ_EXTERNAL_STORAGE) != PackageManager.PERMISSION_GRANTED)
            permissionList.add(Manifest.permission.READ_EXTERNAL_STORAGE);//读取权限
        if (ContextCompat.checkSelfPermission(SurveyPermissionActivity.this, Manifest.permission.WRITE_EXTERNAL_STORAGE) != PackageManager.PERMISSION_GRANTED)
            permissionList.add(Manifest.permission.WRITE_EXTERNAL_STORAGE);//写权限
        if (ContextCompat.checkSelfPermission(SurveyPermissionActivity.this, Manifest.permission.CAMERA) != PackageManager.PERMISSION_GRANTED)
            permissionList.add(Manifest.permission.CAMERA);//相机权限
        ...
        if (permissionList.size() != 0) {
            ...
            ActivityCompat.requestPermissions(SurveyPermissionActivity.this,
                    permissionList.toArray(new String[permissionList.size()]), PERMISSIONS_CAMERA);//打开相机
    ...
    protected void obtainLocAuthor() {
        ...
        if (ContextCompat.checkSelfPermission(this, Manifest.permission.ACCESS_FINE_LOCATION) != PackageManager.PERMISSION_GRANTED) {
            //请求权限
            if (ActivityCompat.shouldShowRequestPermissionRationale(SurveyPermissionActivity.this,
                    Manifest.permission.ACCESS_FINE_LOCATION)) {
                Toast.makeText(SurveyPermissionActivity.this, "请前往设置界面打开定位权限", Toast.LENGTH_SHORT).show();
            } else {
